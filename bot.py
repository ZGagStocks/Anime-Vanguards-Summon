...
# User summon data (store pity counters)
user_summon_data = {}
...
async def summon(interaction: discord.Interaction, amount: int, shinyhunter: bool = False):
    ...
    data = user_summon_data.setdefault(user_id, {
        "mythic_pity_counter": 400,
        "legendary_pity_counter": 50,
    })
    ...
    for _ in range(amount):
        if data["mythic_pity_counter"] <= 0:
            rarity = "Mythic"
            mythic_pity_used = True
            data["mythic_pity_counter"] = 400  # reset mythic pity
            # legendary pity counter unchanged
        elif data["legendary_pity_counter"] <= 0:
            rarity = "Legendary"
            legendary_pity_used = True
            data["legendary_pity_counter"] = 50  # reset legendary pity
            # mythic pity counter unchanged
        else:
            rarity = roll_rarity()
            data["mythic_pity_counter"] -= 1
            data["legendary_pity_counter"] -= 1
    ...
    summons_until_legendary = max(0, data["legendary_pity_counter"])
    summons_until_mythic = max(0, data["mythic_pity_counter"])
